...
- npm
- git
- (선택) Claude Code CLI — 스킬 기능 사용 시 필요
...
```bash
# Quick Mode (즉시 검증, 브라우저 자동 열기 생략)
npm start -- --quick "작업 설명"

# 기본 모드 (검증 + 웹 리포트 자동 오픈)
npm start -- "밤새 크롤링 돌려도 되나?"
```
...
모든 결과물은 `.chaos-lab/run-{timestamp}/` 디렉토리에 생성됩니다:

| 파일 | 설명 |
|------|------|
| `seed.json` | 감지된 서비스, 의존성, 환경 가정 |
| `checklist.json` | Risk Score 정렬된 체크리스트 항목 |
| `results.json` | 각 항목의 검증 결과 |
| `report.json` | Verdict + Action Plan |
| `report.html` | 인터랙티브 웹 대시보드 |
| `chaos-lab-report-*.md` | 마크다운 리포트 |
...
- **지원 OS**: macOS, Linux (CommandVariants로 OS별 명령어 자동 분기)
